...
## Trace Hierarchy: Which Files Matter Most

Not all traces are equal. The three files serve different purposes and carry different training signal weight:

```
wcy_traces_v1_clean.jsonl   ← PRIMARY (48 traces)
  type: void_resolution_cycle
  signal: model marks unknown → investigates → resolves
  why it matters: teaches structured epistemic humility
  avg ?tags: 5.2  avg resolution: 96%

wcy_void_cycles.jsonl        ← SECONDARY (6 traces)
  type: void_resolution_cycle (hand-crafted, higher quality)
  signal: same as above, more carefully constructed
  includes: void cascade (resolution generates new ?tags)

wcy_reasoning_traces.jsonl   ← SUPPLEMENTARY (6 traces)
  type: reasoning (no void-B)
  signal: structured WCY reasoning without uncertainty marking
  use: baseline for format compliance, not epistemic behavior
```

**The void-B resolution cycle is the core contribution.** A model trained only
on `wcy_reasoning_traces.jsonl` learns to format WCY correctly. A model trained
on `wcy_traces_v1_clean.jsonl` learns to represent the boundary of its own
knowledge and act from that boundary.

The difference:
```
# Reasoning trace (wcy_reasoning_traces.jsonl):
: diagnosis=influenza  conf=0.78  from=2,3   ← confident conclusion

# Void cycle trace (wcy_traces_v1_clean.jsonl):
: ?diagnosis  hint=fever+cough+duration  conf_range=0.4..0.8  ← marks unknown
> order  rapid_flu_test  reason=from=4                         ← directed action
. test_result=positive
: diagnosis=influenza  conf=0.94  from=4,6                    ← resolved
```

## Contributing Traces

Additional traces are generated via `experiments/wcy_phase5_pipeline.py`.
To contribute:

1. Set `RUNS_PER_COMBO` in the pipeline to generate more traces (RUNS_PER_COMBO=10 generated 480 additional traces)
2. All three quality gates must pass (parse_r ≥ 0.70, void_generated ≥ 1, resolution_rate ≥ 0.50)
3. Submit traces as JSONL with the same schema, appended to `wcy_traces_v1_clean.jsonl`

Priority domains for future traces: multi-turn dialogue, mathematical proofs,
cross-domain analogy, and adversarial claim verification.

## Version history

- v1.0 (2026-03-17): Initial release. 60 traces, 8 domains.
  Pipeline: 48 traces (Phase 5, RUNS_PER_COMBO=1). Hand-crafted: 12 traces (Phase 4).
- v1.1 (2026-03-18): Scale-up. 540 traces total.
  Pipeline: 528 traces (Phase 5, RUNS_PER_COMBO=10). 480/480 usable (100%). Avg resolution: 95.8%.
